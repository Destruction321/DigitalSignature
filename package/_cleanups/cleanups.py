...
class CleanUps:
    """数字签名窗口清理模块"""
    def __init__(self, root: tk.Tk, dir_labels: dict[DirType, ttk.Label]) -> None:
    ...
    def cleanup_all_files(self) -> None:
        ...
        if selected_days is not None:
            ...
            cleanup_result = _cleanup_utils.cleanup_all_files(
    ...
    def cleanup_temp_files(self) -> None:
        """清理临时文件"""
        cleanup_result = _cleanup_utils.cleanup_temp_files()
        self.__handle_cleanup_result(cleanup_result)

    def cleanup_orphaned_keys(self) -> None:
        """清理孤立的密钥文件"""
        cleanup_result = _cleanup_utils.cleanup_orphaned_keys()
        self.__handle_cleanup_result(cleanup_result)
    ...
    def cleanup_old_files(self) -> None:
        ...
        if selected_days is not None:
            self.__cleanup_days_threshold = selected_days
            cleanup_result = _cleanup_utils.cleanup_old_files(
